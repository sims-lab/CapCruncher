...
def add_agg_stats_args(subcommand):
    ...
    parser = subcommand.add_parser('aggregate_stats',
                                   help='Combine run statistics')

    parser.add_argument('--deduplication_stats', nargs='+',
                   help='Deduplication stats paths', required=True )
    parser.add_argument('--digestion_stats', nargs='+',
                   help='Digestion stats paths', required=True)
...
def add_ccanalyser_args(subcommand):
    parser = subcommand.add_parser('ccanalyser',  help='Analysis of Capture-C processed BAM file')
...
def add_convert_tsv_to_bedgraph_args(subcommand):
    ...
    parser = subcommand.add_parser('ccanalyser_to_bedgraph',
                                    help='Converts ccanalyser output to bedgraph')
...
def add_deduplicate_fastq_args(subcommand):
    ...
    parser = subcommand.add_parser('deduplicate_fastq',
                                    help='Removes PCR duplicates from fastq file')
...
def add_digest_fastq_args(subcommand):
    ...
    parent_parser.add_argument('-o', '--output_file', help='output file name',
    ...
    parent_parser.add_argument('-m', '--minimum_slice_length', help='Shortest length for a slice to be output',
                        default=20, type=int)
    parent_parser.add_argument('--stats_file',
                        help='stats_file_prefix', default='stats.log')
    parent_parser.add_argument('-c', '--compression_level', help='Level of gzip compression (1-9 with 9 being the most compressed/slowest)',
                        default=6, type=int)
    parent_parser.add_argument('--keep_cutsite', help='Determines if cutsite is stripped from the start of each slice',
    ...
    parser = subcommand.add_parser('digest_fastq',
                                    help='Performs in silico digestion of fastq files',
                                    )
...
def add_digest_genome_args(subcommand):
    ...
    parser = subcommand.add_parser('digest_genome',
                                    help='Identifies all restriction fragments in a given genome')
...
def add_join_tsv_args(subcommand):
    parser = subcommand.add_parser('join_tsv',  help='Concatenates or joins multiple tsv files')
...
def add_validation_args(subcommand):
    parser = subcommand.add_parser('validate_annotations',  help='Validates ccanalyser annotation input')
...
def main():
    ...
    if subcommand == 'aggregate_stats':
        from capturec import aggregate_statistics
        aggregate_statistics.main(**vars(args))
    elif subcommand == 'ccanalyser':
        from capturec import ccanalyser
        ccanalyser.main(**vars(args))
    elif subcommand == 'ccanalyser_to_bedgraph':
        from capturec import convert_tsv_to_bedgraph
        convert_tsv_to_bedgraph.main(**vars(args))
    elif subcommand == 'deduplicate_fastq':
