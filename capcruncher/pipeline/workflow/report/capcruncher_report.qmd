...
from capcruncher.api.statistics import FastqDeduplicationStatistics, FastqTrimmingStatistics
...
dedup_stats = pd.DataFrame([s.model_dump() for s in stats])
...
table = pn.widgets.Tabulator(
    dedup_stats[["sample", "total", "unique", "duplicates", "percentage"]]
...
df = dedup_stats[["sample", "duplicates", "unique"]].melt(
...
trimming_stats = [
    FastqTrimmingStatistics(**json.loads(entry)) for entry in load_json(fastq_trimming_path)
]
...
flash_stats = [
    FlashStats(**json.loads(entry)) for entry in load_json(fastq_flash_path)
]
...
digestion_stats = [
    DigestionStats(**load_json(f)) for f in pathlib.Path(fastq_digestion_path).glob("*.json")
]
...
fig = px.line(
    ...
    animation_frame="sample",
    markers=True,
    range_y=(0, df_digestion_read["count"].max() + df_digestion_read["count"].max() * 0.1),
    template="plotly_white",
    )
...
pn.Tabs(("Table", table), ("Digestion Filtering Plot", fig_line), ("Slice Length Histogram", fig_hist)).servable()
```
