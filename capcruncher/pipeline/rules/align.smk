def get_bam_partitions(wc):
    bams = expand(
        "capcruncher_aligned/{sample}/{sample}_part{part}_{combined}.sorted.bam",
        sample=[
            wc.sample,
        ],
...
rule align_bowtie2:
    ...
    output:
        bam=temp("capcruncher_aligned/{sample}/{sample}_part{part}_{combined}.bam"),
    params:
...
rule merge_bam_partitions:
    ...
    output:
        bam="capcruncher_aligned/{sample}.bam",
    shell:
        """
...
rule index_bam:
    ...
    output:
        bam="capcruncher_aligned/{sample}.bam.bai",
    shell:
        """
