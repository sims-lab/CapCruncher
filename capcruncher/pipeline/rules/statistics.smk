...
def get_digestion_statistics(wc):
    stat_types = {"read_level_stats": "digestion.read.summary.csv",
    ...
    for sample in SAMPLE_NAMES:
        for part in get_fastq_partition_numbers_for_sample(wc, sample_name=sample):
            for combined in ["flashed", "pe"]:
                stat_prefixes.append(f"capcruncher_statistics/digestion/data/{sample}_part{part}_{combined}.")
...
def get_filtering_statistics(wc):
    stat_types = {"read_level_stats": "read.stats.csv",
    ...
    for sample in SAMPLE_NAMES:
        for part in get_fastq_partition_numbers_for_sample(wc, sample_name=sample):
            for combined in ["flashed", "pe"]:
                stat_prefixes.append(f"capcruncher_statistics/filtering/data/{sample}_part{part}_{combined}.")
...
rule combine_stats_alignment_deduplication:
    input:
        read_level_stats=expand("capcruncher_statistics/deduplication_by_coordinate/data/{sample}_{combined}.read.stats.csv",
                          sample=SAMPLE_NAMES,
                          combined=["flashed", "pe"]),
    output:
...
rule combine_stats_cis_and_trans:
    input:
        cis_and_trans_stats=expand("capcruncher_statistics/cis_and_trans_reporters/data/{sample}_{combined}.reporter.stats.csv",
                                   sample=SAMPLE_NAMES,
                                   combined=["flashed", "pe"]),
    output:
