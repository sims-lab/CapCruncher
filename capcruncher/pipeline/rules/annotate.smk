...
def configure_annotation_parameters():
    ...
    defaults_file = workflow.source_path("annotation_defaults.json")
...
def format_priority_chromosome_list():
    ...
    if chromosomes:
        chromosomes = f"--priority-chroms {chromosomes}"
    else:
        chromosomes = ""
...
rule exclusions:
    ...
    output:
        exclusions="capcruncher_annotate/exclude.bed",
    params:
...
rule annotate:
    input:
        ...
        viewpoints=config["analysis"]["viewpoints"],
    output:
        annotated="capcruncher_annotate/{sample}/{sample}_part{part}_{combined}.parquet",
    params:
