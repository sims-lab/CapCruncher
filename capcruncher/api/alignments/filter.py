...
class AlignmentFilterOptions(BaseModel):
    ...
    @field_validator("filter_profile", mode="before")
    @classmethod
    def validate_filter_profile(cls, value: Path | str | None) -> Path | None:
        if value in (None, ""):
            return None
        return existing_path(value, "filter_profile")
...
def filter(
    ...
    Args:
        ...
        filter_profile: Optional TOML file defining filter stage order.
    ...
    with logger.catch():
        ...
        slice_filter = slice_filter_class(
            slices=df_alignment,
            sample_name=options.sample_name or "",
            read_type=read_type.value,
            filter_profile=options.filter_profile,
        )
