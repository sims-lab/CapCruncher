...
parser.add_argument('-m', '--minimum_slice_length', help='Shortest length for a slice to be output',
               default=0, type=int)
parser.add_argument('-l', '--logfile', help='logfile_prefix', default=sys.stdout)
    ...
    def get_slices(self):
        ...
        for site in self.recognition_sites:
            ...
            if slice_length >= self.min_slice_len and slice_end != 0:
                ...
                if not self.flashed: # If the reads are not flashed then keep them even if they do not digest
                    self.slices_valid_counter += 1
                    slices_lst.append(s)
                elif self.flashed and slice_length < len(self.read.sequence):
                    self.slices_valid_counter += 1
                    slices_lst.append(s)
...
def main():
    ...
    with gzip.open(args.output_file, 'w', compresslevel=args.compression_level) as fastq_out: 
        ...
        for read_type in slice_total_dict:
            ...
            if total_slices:
                stats = {'total_read_pairs_processed': seq_counter,
                         'total_slices': sum(k * v for k, v in slice_total_dict[read_type].items()),
                         'total_valid_slices': sum(k * v for k, v in slice_valid_dict[read_type].items())}
                
                hist = {f'frequency_of_{k}_slices': slice_valid_dict[read_type][k] for k in sorted(slice_valid_dict[read_type])}
                stats.update(hist)
            else:
