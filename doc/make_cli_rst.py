...
def _format_description(ctx):
    ...
    doc_string = GoogleDocstring(help_string)
...
commands = _get_lazyload_commands(cli)

ctx_parent = click.Context(cli)
formatted_cmd = _format_command(ctx_parent, 'full')
breakpoint()
print(''.join(list(formatted_cmd)))
